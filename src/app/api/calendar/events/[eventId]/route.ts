...
export async function PATCH(request: Request, { params }: CalendarEventRouteParams) {
	...
	if (!input.title?.trim() || Number.isNaN(startsAt.getTime()) || Number.isNaN(endsAt.getTime()) || endsAt <= startsAt) return NextResponse.json({ error: t("validEventRequired") }, { status: 400 });
	const db = getDb(env);
	...
	if (!existing) return NextResponse.json({ error: t("eventNotFound") }, { status: 404 });
