...
export default function SettingsExportPage() {
	...
	const tExport = useTranslations("importExport");
	...
	async function onExport() {
		...
		try {
			...
			setExportState({ error: error instanceof Error ? error.message : tExport("exportFailed"), loading: false });
			return;
		}
	...
	return (
		<div className="space-y-6">
			...
			<section className="space-y-4">
				<div>
					<h2 className="text-xl font-semibold text-neutral-900">{t("exportTitle")}</h2>
					<p className="mt-1 text-sm text-neutral-500">
						{t("exportDescription")}
					</p>
				</div>
				<div className="space-y-3 rounded-3xl bg-white p-6">
					<Button type="button" variant="outline" disabled={!selectedMailbox || exportState.loading} onClick={onExport}>
						{exportState.loading ? tExport("exportPreparing") : tExport("exportDownload")}
					</Button>
					{exportState.error && (
