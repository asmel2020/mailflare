...
export function CurrentMailboxForm() {
	...
	async function onSubmit(event: FormEvent<HTMLFormElement>) {
		...
		try {
			...
			setStatus(err instanceof Error ? err.message : t("failedToUpdateMailbox"));
		} finally {
			setSaving(false);
		}
	}
	...
	if (!selectedMailbox) {
		return (
			<div className="space-y-6">
				<h1 className="text-3xl font-medium text-neutral-900">{t("settingsTitle")}</h1>
				<Card className="rounded-3xl border-0 bg-white p-6">
					<CardContent className="p-6 text-sm text-neutral-500">
						{t("selectMailboxToView")}
					</CardContent>
				</Card>
			</div>
		);
