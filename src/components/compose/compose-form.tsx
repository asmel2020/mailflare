...
export function ComposeForm({
	...
	useEffect(() => {
		...
		fetchDraft(draftIdToLoad)
			...
			.catch((err) => {
				if (cancelled) return;
				const message = err instanceof Error ? err.message : t("loadDraftFailed");
	...
	async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
		...
		if (to.length === 0) {
			setToast({ type: "error", message: t("addRecipient") });
			return;
		}
		const invalid = [...to, ...cc, ...bcc].find((entry) => !isValidRecipient(entry));
		if (invalid) {
			setToast({ type: "error", message: t("invalidEmail", { email: invalid }) });
			return;
		}
		if (!hasMeaningfulHtml(html) && !quotedHtml) {
			setToast({ type: "error", message: t("writeMessage") });
			return;
		}
		setLoading(true);
		...
		setAttachments([]);
		setScheduledAt(null);
		setToast({ type: "success", message: data.scheduled ? t("messageScheduled") : t("messageSent") });
		window.dispatchEvent(new Event("mailflare:messages-changed"));
	}
	...
	async function deleteDraftAndClose() {
		...
		if (draftId) {
			...
			if (!res.ok) {
				...
				setToast({ type: "error", message: t("deleteDraftFailed") });
				return;
			}
		}
	...
	async function removeStoredAttachment(attachmentId: string) {
		...
		if (!res.ok) {
			setToast({ type: "error", message: t("removeAttachmentFailed") });
			return;
		}
	...
	function addAttachments(files: FileList | null) {
		...
		if (nextCount > 10) {
			setToast({ type: "error", message: t("tooManyAttachments") });
			return;
		}
		if (nextFiles.some((file) => file.size > 10 * 1024 * 1024)) {
			setToast({ type: "error", message: t("attachmentTooLarge") });
			return;
		}
		if (totalSize > 20 * 1024 * 1024) {
			setToast({ type: "error", message: t("attachmentsTooLarge") });
			return;
		}
		...
		<>
			...
			<form onSubmit={onSubmit} className={frameClass}>
				...
				<div className="border-b border-neutral-100 px-4 py-1 flex flex-row items-center">
					...
					>
						{senderOptions.length === 0 && <option value="">{t("selectMailboxFirst")}</option>}
