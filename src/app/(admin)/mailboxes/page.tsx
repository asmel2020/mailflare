...
export default function MailboxesPage() {
	...
	if (account.data?.user?.id && !mailboxOwners.some((owner) => owner.id === account.data?.user?.id)) {
		mailboxOwners.unshift({
			...
			name: account.data.user.name ?? account.data.user.email ?? t("currentAccount"),
			role: "admin",
		});
	}
				...
				<Dialog open={createOpen} onOpenChange={setCreateOpen}>
					...
					<DialogContent>
						<DialogHeader>
							...
							<DialogDescription>{t("createMailboxDescription")}</DialogDescription>
						</DialogHeader>
						<div className="space-y-4">
							{mailboxes.data?.canCreateShared && (
								<div className="space-y-2">
									...
									>
										<option value="personal">{t("personalInbox")}</option>
										<option value="shared">{t("sharedInbox")}</option>
									</Select>
								</div>
							)}
							...
							<div className="space-y-2">
								<Label htmlFor="mailbox-username">{t("emailAddress")}</Label>
							...
							>
								{create.isPending ? t("creating") : t("createButton")}
							</Button>
						</div>
					</DialogContent>
				</Dialog>
								...
								<span className="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-blue-100 text-sm font-semibold text-blue-700">
									...
									{mailbox.hasAvatar && (
										<img
											...
											alt={t("profileAlt", { name: getMailboxName(mailboxWithHostname) })}
